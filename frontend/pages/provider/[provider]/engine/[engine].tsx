...
const Provider: NextPage<Props> = ({
  ...
  useEffect(() => {
    ...
    // For engine type selector, ditto.
    updateSearchConfig("engineType", [engine.toUpperCase() as EngineType]);
  }, [engine, provider, updateSearchConfig]);
  ...
  return (
    ...
    >
      <div className="mx-5 mt-4 pb-2">
        ...
        <SearchMenu hideProviders={true} hideEngineType={true} />
...
export const getStaticPaths = () => ({
  paths: [
    { params: { provider: "aws", engine: "mysql" } },
    { params: { provider: "aws", engine: "postgres" } },
    { params: { provider: "gcp", engine: "mysql" } },
    { params: { provider: "gcp", engine: "postgres" } },
  ],
  fallback: false,
});

export const getStaticProps: GetStaticProps = async (context) => {
  const { provider, engine } = context.params as unknown as Params;
  ...
  const firstPageData = data
    .filter((instance) => instance.cloudProvider === provider.toUpperCase())
